...
- 2025-11-20 12:05: Added a collapse/expand affordance to the Track Summary card so users can hide the long synopsis/sections, tightened vertical padding inside each section row, and reduced the card’s outer padding to match the new compact design.
- 2025-11-20 17:05: Fixed an `EXC_BAD_ACCESS` crash during summary streaming by letting `AIGenerationJobExecutor` own per-job stream buffers (actor-isolated dictionary + helper) and updating closures to call an async `persistStreamDelta` helper instead of mutating a local `String` from detached tasks. This keeps stream updates serialized on the actor and eliminates the frozen-screen repro.
