...
## Scope
- **Included**: GRDB/SQLite databases (including listening statistics - session-level data tracking when and how long users listened to each track), transcript files (text + timing only), playback snapshots/history, Settings/AppStorage toggles, cache TTL preference, Baidu folder shortcuts, favorites, AI/TTS configuration (but not raw API keys unless user opts in).
...
| Listening statistics | `GRDBDatabaseManager` (library DB, `listening_statistics` table) | `database/library.sqlite` | Session-level data: track_id, collection_id, start_time, end_time, created_at |
