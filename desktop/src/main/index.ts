...
import { fetchWithElectronNet } from './media-net'

protocol.registerSchemesAsPrivileged([
  { scheme:'hb-media', privileges:{standard:true,secure:true,supportFetchAPI:true,stream:true} },
  { scheme:'hb-cover', privileges:{standard:true,secure:true,supportFetchAPI:true} }
])
...
function registerIpc(){
 ipcMain.handle(IPC_CHANNELS.getStatus,()=>library.status()); ipcMain.handle(IPC_CHANNELS.refresh,()=>library.refresh()); ipcMain.handle(IPC_CHANNELS.getFolders,()=>library.folders()); ipcMain.handle(IPC_CHANNELS.getFavorites,()=>library.favorites()); ipcMain.handle(IPC_CHANNELS.getCollections,(_e,folderId:string|null,includeArchived?:boolean)=>library.collections(folderId,includeArchived)); ipcMain.handle(IPC_CHANNELS.getTracks,(_e,id:string,page?:number,size?:number)=>library.tracks(id,page,size)); ipcMain.handle(IPC_CHANNELS.getContinue,(_e,limit?:number)=>library.continueListening(limit)); ipcMain.handle(IPC_CHANNELS.search,(_e,q:string,limit?:number)=>library.search(q,limit)); ipcMain.handle(IPC_CHANNELS.getText,(_e,id:string)=>library.textContent(id)); ipcMain.handle(IPC_CHANNELS.resolvePlayback,(_e,id:string)=>library.media(id)); ipcMain.handle(IPC_CHANNELS.baiduStatus,()=>baidu.getStatus()); ipcMain.handle(IPC_CHANNELS.baiduLogin,()=>baidu.login(mainWindow)); ipcMain.handle(IPC_CHANNELS.baiduLogout,()=>baidu.logout())
}
app.whenReady().then(()=>{ library=new LibraryService(snapshot);snapshot.initialize();registerIpc();setInterval(()=>snapshot.refresh(false),30000);protocol.handle('hb-cover',request=>{try{const u=new URL(request.url);const relative=decodeURIComponent(`${u.hostname}${u.pathname}`).replace(/^\/+/, '');const root=dirname(SOURCE_DB);const file=join(root,relative);if(!(file===root||file.startsWith(root+sep))||!existsSync(file))return new Response('Not found',{status:404});return net.fetch(pathToFileURL(file).toString())}catch{return new Response('Bad request',{status:400})}});protocol.handle('hb-media',async request=>{try{const id=new URL(request.url).hostname;const source=library.media(id);if(!source)return new Response('Not found',{status:404});if(source.kind==='baidu')return baidu.stream(source.value,request);const url=new URL(source.value);if(!['http:','https:'].includes(url.protocol))return new Response('Unsupported URL',{status:400});if(url.protocol==='http:')url.protocol='https:';const range=request.headers.get('Range');return fetchWithElectronNet(url.toString(),range?{Range:range}: {})}catch(error){return new Response(String(error),{status:502})}});createWindow()})
