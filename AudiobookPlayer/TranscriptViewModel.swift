...
class TranscriptViewModel: NSObject, ObservableObject {
    ...
    @Published var translations: [TrackSummaryTranslation] = [] {
        didSet { rebuildTranslationLookup() }
    }
    ...
    private var translationLookup: [Int: TrackSummaryTranslation] = [:]
    ...
    func loadTranscript() async {
        ...
        do {
            ...
            if let loadedTranscript = try await dbManager.loadTranscript(forTrackId: trackId) {
                ...
                // Load any lyric translations generated by track summary jobs
                translations = try await loadTranslations()
    ...
    /// Find the best matching translation for a given transcript segment, if available.
    func translation(for segment: TranscriptSegment) -> TrackSummaryTranslation? {
        guard !translations.isEmpty else { return nil }

        let start = segment.startTimeMs
        if let exact = translationLookup[start] {
            return exact
        }

        // Allow slight drift between translation start times and transcript segments.
        let toleranceMs = 1500
        return translations.min { lhs, rhs in
            abs(lhs.startTimeMs - start) < abs(rhs.startTimeMs - start)
        }.flatMap { candidate in
            abs(candidate.startTimeMs - start) <= toleranceMs ? candidate : nil
        }
    }
    ...
    private func loadTranslations() async throws -> [TrackSummaryTranslation] {
        ...
        return summary.translations.sorted { lhs, rhs in
    ...
    private func rebuildTranslationLookup() {
        translationLookup = Dictionary(
            translations.map { ($0.startTimeMs, $0) },
