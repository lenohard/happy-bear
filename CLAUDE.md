...
## Recent Progress (2025-12-08 – 2025-12-15)
- **CollectionReviewView Sort Toggle (2025-12-15)**: Added a sort toggle button to `CollectionReviewView` to switch between ascending and descending order (defaulting to ascending/input order). This allows users to easily reverse the track list, which is particularly useful for RSS feeds where the default order is by publication date.
- **CollectionDetailView MVVM Refactoring (2025-12-15)**: Refactored `CollectionDetailView` to use dedicated `CollectionDetailViewModel` following MVVM pattern, separating presentation logic from view code. Fixed actor isolation error by removing unused `pageIndexForTrack` method that called actor-isolated database methods synchronously. Commits: `5eabfe2`, `c987720`.
...
## Recent Lessons
- **2025-12-15 – Actor isolation in ViewModels**: When calling actor-isolated methods (e.g., GRDB database queries) from non-async ViewModel methods, Swift will throw actor isolation errors. Solution: Either make the ViewModel method `async` and properly await the actor call, or remove the synchronous wrapper entirely if it's unused. Example: `pageIndexForTrack()` was removed from `CollectionDetailViewModel` because it synchronously called `GRDBDatabaseManager.shared.fetchTrackNumber()` which is actor-isolated. Commit: `c987720`.
- **2025-12-15 – CollectionDetailView refactoring**: `CollectionDetailView` now uses a dedicated `CollectionDetailViewModel` following the MVVM pattern, separating presentation logic from view code. This improves testability and makes actor isolation boundaries clearer. Commit: `5eabfe2`.
...
- **2025-12-09 – Baidu Netdisk Collection Refresh Behavior**:
  ...
  - **Renaming the source folder**: Breaks the refresh feature. Collections store a fixed path string (e.g., `/Apps/Book`). If the folder is renamed in Baidu Netdisk, the stored path becomes invalid, causing refresh to fail. Existing tracks may still play via `fs_id`, but new file detection will stop working.
- **2025-12-09 – Baidu Netdisk WebM Support**: Baidu Netdisk does not natively support WebM streaming. While `PlayableMediaFormat.swift` allows `.webm` files to be treated as video, direct streaming from Baidu will likely fail without transcoding or using a specialized third-party player that handles the raw stream differently.
